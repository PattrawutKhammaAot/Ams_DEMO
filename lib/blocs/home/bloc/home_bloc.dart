...
class HomeBloc extends Bloc<HomeEvent, HomeState> {
  HomeBloc._() :super(HomeInitial()) {
    ...
    on<HomeEvent_LoadCountDashboard>((event, emit) async {
      try {
        ...
        var response = await apiController.getData('Count/GetCountDashboardCountPlan',"", useAuth: true);
  ...
  FutureOr<void> _onScrollChanged(HomeScrollChanged event, Emitter<HomeState> emit) {
    if (event.scroll) {
      if (kDebugMode) {
        print('isNotScroll');
      }
